...
client="./target/debug/neqo-client $flags --no-sni-slicing --output-dir $tmp https://$addr:$port$path --stats"
server="SSLKEYLOGFILE=$tmp/test.tlskey ./target/debug/neqo-server $flags $addr:$port --stats"
