...
use neqo_common::{Buffer, Decoder, Encoder, hex::HexWithLen, qdebug};
...
impl HFrameType {
    ...
    /// recognized only to log and ignore them when a peer sends them. See [`HFrame::decode`].
...
impl FrameDecoder<Self> for HFrame {
    ...
    fn max_frame_data(frame_type: HFrameType) -> usize {
        match frame_type {
            HFrameType::HEADERS => MAX_HEADER_BYTES,
            HFrameType::GOAWAY => MAX_SINGLE_VARINT_FRAME_BYTES,
            HFrameType::SETTINGS | HFrameType::PRIORITY_UPDATE_REQUEST => MAX_BUFFERED_FRAME_BYTES,
            _ => usize::MAX,
        }
    }
    ...
    fn decode(frame_type: HFrameType, frame_len: u64, data: Option<&[u8]>) -> Res<Option<Self>> {
        if frame_type == HFrameType::DATA {
            ...
            // Server push is not supported. Log and ignore the frame (it is discharged like any
            // unknown frame, i.e. `is_known_type` stays false).
            qdebug!(
                "Ignoring HTTP/3 server push frame (type {:#x})",
                frame_type.0
            );
            Ok(None)
