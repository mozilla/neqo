...
impl FrameReaderTest {
    ...
    fn process<T: FrameDecoder<T>>(&mut self, v: &[u8]) -> Option<T> {
        ...
        let out = self.conn_s.process_alloc(None, now());
        mem::drop(self.conn_c.process_alloc(out.as_dgram_ref(), now()));
        let (frame, fin) = self
            .fr
...
fn test_reading_frame<T: FrameDecoder<T> + PartialEq + Debug>(
    ...
    let out = fr.conn_s.process_alloc(None, now());
    mem::drop(fr.conn_c.process_alloc(out.as_dgram_ref(), now()));
...
fn complete_and_incomplete_frames() {
    ...
    // TODO: separate write buffer needed?
    let mut write_buffer = Vec::with_capacity(2);
    let mut enc = Encoder::new_with_buffer(&mut write_buffer);
    f.encode(&mut enc);
...
fn frame_reading_when_stream_is_closed_before_sending_data() {
    ...
    let out = fr.conn_c.process_alloc(None, now());
    mem::drop(fr.conn_s.process_alloc(out.as_dgram_ref(), now()));
    assert_eq!(
        Ok((None, true)),
        fr.fr
