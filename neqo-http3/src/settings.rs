...
impl ZeroRttChecker for HttpZeroRttChecker {
    fn check(&self, token: &[u8]) -> ZeroRttCheckResult {
        ...
        if settings.iter().all(|setting| match setting.setting_type {
            ...
            //
            // `save()` below never writes WtInitialMax* into a token (only the
            // draft-15 enable flag is saved), so these arms are unreachable
            // for tokens generated by this implementation. They only exist to
            // keep this match exhaustive; revisit if `save()` starts
            // persisting per-session limits, at which point 0-RTT should be
            // rejected if the server has since lowered a limit below what the
            // token assumed (mirroring the QPACK capacity check above).
            HSettingType::EnableWebTransportDraft07
            | HSettingType::WtInitialMaxData
            | HSettingType::WtInitialMaxStreamsUni
            | HSettingType::WtInitialMaxStreamsBidi
