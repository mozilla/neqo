...
const SETTINGS_ENABLE_CONNECT_PROTOCOL: SettingsType = 0x08;
            ...
            for iter in &self.settings {
                match iter.setting_type {
                    ...
                    HSettingType::EnableConnect => {
                        // TODO: We don't need to do this as a client.
    ...
    pub fn decode_frame_contents(&mut self, dec: &mut Decoder) -> Res<()> {
        while dec.remaining() > 0 {
            ...
            match (t, v) {
                ...
                (Some(t), Some(v)) => {
                    println!("Ignoring unknown setting type {t} with value {v}"); // ignore unknown setting, it is fine.
