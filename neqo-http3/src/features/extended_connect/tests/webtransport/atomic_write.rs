...
use crate::{Error, features::extended_connect::tests::webtransport::WtTest};
...
fn wt_flow_control_info_buffered_then_cleared() {
    ...
    let fc = wt
        ...
        .unwrap();
    assert!(
        fc.buffered >= SMALL_DATA.len(),
        "queued data should appear as buffered (got {})",
        fc.buffered
    );
...
fn wt_atomic_send_too_large_for_window() {
    ...
    let fc = wt
        ...
        .webtransport_send_stream_flow_control_info(stream_id)
        .unwrap();
    if fc.available >= SMALL_DATA.len() {
        assert!(
            wt.client
                .webtransport_send_stream_atomic(stream_id, SMALL_DATA, now())
                .unwrap()
