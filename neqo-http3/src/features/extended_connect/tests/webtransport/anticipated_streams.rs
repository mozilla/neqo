...
fn anticipated_uni_invalid_session() {
    ...
    assert_eq!(
        wt.client
            .webtransport_set_anticipated_incoming_uni_streams(bogus, 10),
...
fn anticipated_bidi_invalid_session() {
    ...
    assert_eq!(
        wt.client
            .webtransport_set_anticipated_incoming_bidi_streams(bogus, 10),
        Err(Error::InvalidStreamId)
    );
}
...
fn anticipated_uni_valid_session() {
    ...
    assert_eq!(
        wt.client
            .webtransport_set_anticipated_incoming_uni_streams(session.stream_id(), 10),
        Ok(())
    );
}
...
fn anticipated_bidi_valid_session() {
    ...
    assert_eq!(
        wt.client
            .webtransport_set_anticipated_incoming_bidi_streams(session.stream_id(), 10),
        Ok(())
    );
}
...
fn anticipated_uni_before_session_negotiated() {
    ...
    let session_id = wt
        ...
        .unwrap();
    wt.client
        .webtransport_set_anticipated_incoming_uni_streams(
            session_id,
...
fn anticipated_bidi_above_default_sends_frame() {
    ...
    wt.client
        .webtransport_set_anticipated_incoming_bidi_streams(
            session.stream_id(),
...
// A session that has been closed is no longer active; setting its anticipated
// streams must be rejected like any other invalid session ID.
#[test]
fn anticipated_uni_rejected_after_session_closed() {
    ...
    assert_eq!(
        wt.client
            .webtransport_set_anticipated_incoming_uni_streams(session_id, 10),
        Err(Error::InvalidStreamId)
    );
}
...
fn anticipated_bidi_rejected_after_session_closed() {
    ...
    assert_eq!(
        wt.client
            .webtransport_set_anticipated_incoming_bidi_streams(session_id, 10),
        Err(Error::InvalidStreamId)
    );
}
...
fn anticipated_uni_total_is_clamped() {
    ...
    wt.client
        .webtransport_set_anticipated_incoming_uni_streams(session_a.stream_id(), u16::MAX)
        .unwrap();
    wt.exchange_packets();
    let after_a = client_max_streams_sent(&wt);
    assert!(after_a > 0, "session A should trigger a MAX_STREAMS frame");
    ...
    wt.client
        .webtransport_set_anticipated_incoming_uni_streams(session_b.stream_id(), u16::MAX)
        .unwrap();
    wt.exchange_packets();
...
fn lets_peer_open_more_streams(stream_type: StreamType) {
    const BUF: &[u8] = &[0; 10];

    let default_limit = stream_limit_default(stream_type);
    let mut wt = WtTest::new();
    ...
    // The server opens one more stream than the transport default would have allowed.
    // For unidirectional streams HTTP/3's own control streams share the same limit, so
    // going past the default at all requires the raised limit to have taken effect.
    for _ in 0..=default_limit {
        let stream = WtTest::create_wt_stream_server(&wt_session, stream_type);
...
fn anticipated_uni_lets_peer_open_more_streams() {
    lets_peer_open_more_streams(StreamType::UniDi);
}

#[test]
fn anticipated_bidi_lets_peer_open_more_streams() {
    lets_peer_open_more_streams(StreamType::BiDi);
}
