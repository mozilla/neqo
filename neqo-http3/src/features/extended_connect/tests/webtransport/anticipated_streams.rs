...
fn anticipated_uni_at_default_sends_no_frame() {
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session.stream_id(), STREAM_LIMIT_DEFAULT as u16)
        .unwrap();
    wt.exchange_packets();
...
fn anticipated_uni_above_default_sends_frame() {
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session.stream_id(), STREAM_LIMIT_DEFAULT as u16 + 1)
        .unwrap();
    wt.exchange_packets();
