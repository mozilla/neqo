...
// Connection-level baseline for stream limits (from LOCAL_STREAM_LIMIT_*
// in neqo-transport).  Values at or below this require no MAX_STREAMS frame.
const STREAM_LIMIT_DEFAULT: u16 = 100;
...
fn anticipated_uni_at_default_sends_no_frame() {
    ...
    wt.client
        .webtransport_increase_max_uni_streams(
            session.stream_id(),
            STREAM_LIMIT_DEFAULT - u16::try_from(HTTP3_UNI_CONTROL_STREAMS).unwrap(),
        )
        .unwrap();
    wt.exchange_packets();
...
fn anticipated_uni_above_default_sends_frame() {
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session.stream_id(), STREAM_LIMIT_DEFAULT + 1)
        .unwrap();
    wt.exchange_packets();
...
fn anticipated_bidi_above_default_sends_frame() {
    ...
    wt.client
        .webtransport_increase_max_bidi_streams(session.stream_id(), STREAM_LIMIT_DEFAULT + 1)
        .unwrap();
    wt.exchange_packets();
...
fn anticipated_uni_decrease_sends_no_frame() {
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session.stream_id(), STREAM_LIMIT_DEFAULT + 50)
        .unwrap();
    wt.exchange_packets();
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session.stream_id(), STREAM_LIMIT_DEFAULT + 10)
        .unwrap();
    wt.exchange_packets();
...
fn anticipated_uni_two_sessions_sums_values() {
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session_a.stream_id(), STREAM_LIMIT_DEFAULT + 1)
        .unwrap();
    wt.exchange_packets();
    ...
    wt.client
        .webtransport_increase_max_uni_streams(session_b.stream_id(), STREAM_LIMIT_DEFAULT + 1)
        .unwrap();
    wt.exchange_packets();
...
fn anticipated_bidi_two_sessions_sums_values() {
    ...
    wt.client
        .webtransport_increase_max_bidi_streams(session_a.stream_id(), STREAM_LIMIT_DEFAULT + 1)
        .unwrap();
    wt.exchange_packets();
    let after_a = client_max_streams_sent(&wt);
    assert!(
    ...
    wt.client
        .webtransport_increase_max_bidi_streams(session_b.stream_id(), STREAM_LIMIT_DEFAULT + 1)
        .unwrap();
    wt.exchange_packets();
