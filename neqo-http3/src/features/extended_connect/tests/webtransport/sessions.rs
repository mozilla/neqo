...
fn wt_create_send_group() {
    ...
    let result = wt.client.webtransport_register_send_group(session_id, group);
    assert!(result.is_ok());
...
fn wt_validate_send_group() {
    ...
    let group = SendGroupId::new();
    wt.client.webtransport_register_send_group(session_id, group).unwrap();
...
fn wt_cross_session_send_group_rejected() {
    ...
    wt.client.webtransport_register_send_group(session_id1, group1).unwrap();
...
fn wt_send_group_with_sendorder() {
    ...
    let group1 = SendGroupId::new();
    wt.client.webtransport_register_send_group(session_id, group1).unwrap();
    let group2 = SendGroupId::new();
    wt.client.webtransport_register_send_group(session_id, group2).unwrap();
...
fn wt_send_groups_fair_bandwidth_allocation() {
    ...
    let session_id = wt_session.stream_id();
     ...
     wt.client.webtransport_register_send_group(session_id, group_high).unwrap();
     let group_low = SendGroupId::new();
     wt.client.webtransport_register_send_group(session_id, group_low).unwrap();
...
fn wt_multiple_groups_separate_sendorder_namespaces() {
    ...
    wt.client.webtransport_register_send_group(session_id, group3).unwrap();
...
fn wt_drain_session_client() {
    ...
    wt.server.test_drain_wt_session(session_id, now());
    wt.exchange_packets();
    ...
    assert!(has_draining, "expected Draining event after server WT_DRAIN_SESSION");
    ...
    assert!(!has_closed, "session must remain open after WT_DRAIN_SESSION");
}
...
fn wt_drain_session_server() {
    ...
    wt.client.test_wt_drain_session(session_id, now()).unwrap();
    wt.exchange_packets();
    ...
    assert!(has_draining, "expected server Draining event after client WT_DRAIN_SESSION");
