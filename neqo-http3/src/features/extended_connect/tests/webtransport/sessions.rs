...
fn wt_close_session_cannot_be_sent_at_once() {
    ...
    let out = wt.server.process_alloc(None, now());
    let out = wt.client.process_alloc(out.as_dgram_ref(), now());
    ...
    let out = wt.server.process_alloc(out.as_dgram_ref(), now());
    let out = wt.client.process_alloc(out.as_dgram_ref(), now());
    let out = wt.server.process_alloc(out.as_dgram_ref(), now());
    let out = wt.client.process_alloc(out.as_dgram_ref(), now());
    let out = wt.server.process_alloc(out.as_dgram_ref(), now());
    let _out = wt.client.process_alloc(out.as_dgram_ref(), now());
