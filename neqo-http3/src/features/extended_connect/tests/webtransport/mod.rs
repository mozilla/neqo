...
fn connect_with(client: &mut Http3Client, server: &mut Http3Server) {
    ...
    let out = client.process_alloc(out.as_dgram_ref(), now());
    let out = server.process_alloc(out.as_dgram_ref(), now());
    std::mem::drop(client.process_alloc(out.as_dgram_ref(), now()));
}
...
impl WtTest {
    ...
    fn exchange_packets(&mut self) {
        ...
        loop {
            now += RTT / 2;
            out = self.client.process_alloc(out.as_ref(), now).dgram();
            let client_none = out.is_none();
            now += RTT / 2;
            out = self.server.process_alloc(out.as_ref(), now).dgram();
