...
pub struct SessionStats {
    ...
    ///
    /// Excludes a datagram the transport drops at packet-build time for no
    /// longer fitting the path MTU (`OutgoingDatagramOutcome::DroppedTooBig`,
    /// counted in `Stats::datagram_tx`): the session's queue accepted it, and
    /// nothing reports that fate back to this layer. It cannot happen at all
    /// unless the path MTU shrinks after the datagram was accepted, since
    /// oversized ones are refused before they are ever queued.
