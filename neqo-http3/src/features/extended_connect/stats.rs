...
///
/// Every datagram handed to `send_datagram` and accepted ends up in exactly
/// one of the three counters, so `sent + expired + dropped` equals the number
/// accepted (bar the `DroppedTooBig` case below).  That is why `dropped`
...
pub struct SessionStats {
    ...
    /// still queued when the session closed.
    ///
