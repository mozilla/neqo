...
impl Session {
    ...
    ///   - The protocol doesn't support HTTP DATAGRAM Capsules, and QUIC datagram sending fails
    ///   - OR capsule sending fails and QUIC datagram sending also fails as fallback
    /// - When QUIC datagrams are available: datagram sending fails
    pub(crate) fn send_datagram<I: Into<DatagramTracking>>(
        ...
        if conn.remote_datagram_size() == 0 {
            ...
            if self.protocol.write_datagram_capsule(
            ...
            qtrace!("[{self}] Capsule not supported, falling back to QUIC datagram");
        }
