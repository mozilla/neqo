...
impl SendStream for WebTransportSendStream {
	...
	conn.stream_sendorder(self.stream_id, sendorder).map_err(|_| crate::Error::InvalidStreamId)
    }

    fn set_fairness(&mut self, conn: &mut Connection, fairness: bool) -> Res<()> {
	conn.stream_fairness(self.stream_id, fairness).map_err(|_| crate::Error::InvalidStreamId)
    }
