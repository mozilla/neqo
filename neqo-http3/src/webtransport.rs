...
pub trait ClientSession {
    ...
    ///
    /// The mark is session-wide: it counts every datagram queued for the
    /// session, whichever `WebTransportDatagramsWritable` wrote it, whereas
    /// the spec applies `outgoingMaxBufferedDatagrams` to each writable's own
    /// queue. An embedder with several writables on one session should
    /// either scale the value by the number of live writables or leave it
    /// `None` and apply the per-writable limit itself.
    ///
    /// neqo has no default mark for a `WebTransport` session: `None` is
    /// also the initial state, so the embedder supplies the spec's
    /// implementation-defined initial value through this setter.
    ///
    /// Like [`Self::webtransport_set_datagram_max_age`], this is accepted in
    /// any session state, including before the CONNECT response arrives,
    /// since the `WebIDL` attributes can be set before `ready` resolves.
    ///
    /// # Errors
    ///
    ...
    /// set for all not-yet-closed WebTransport sessions on this connection, capped at 10,000
    /// streams per direction; it does not create a per-session limit. A `value` that would push
    /// the sum past the cap is accepted but only takes effect up to it. The transport limit only
    /// ever rises, so lowering `value` does not reduce it. The limit is a concurrency window,
    /// not a cumulative budget: as the peer closes streams it opened, it may open further
    /// streams without another call here.
    ///
    /// # Errors
    ///
    ...
    ///
    /// Mapping from the `WebIDL` `unrestricted double?`: the spec's setter
    /// turns `0` into null, so the caller must pass `None` for it;
    /// `Some(Duration::ZERO)` is floored to the timer granularity instead.
    /// `+Infinity` ("never expire") is `Some(Duration::MAX)`, which is safe.
    ///
    /// # Errors
    ///
    /// Returns error if the session ID is invalid or is not a WebTransport session.
...
pub(crate) trait ServerHandler {
    ...
    /// See [`ClientSession::webtransport_set_datagram_high_water_mark`].
    ///
    /// # Errors
    ///
    /// Returns error if the session ID is invalid or is not a WebTransport session.
...
impl ServerHandler for Http3ServerHandler {
    ...
    fn webtransport_set_datagram_high_water_mark(
        ...
        self.base_handler_mut()
            .webtransport_session_set_datagram_high_water_mark(conn, session_id, high_water_mark)
    }
...
impl ServerSession {
    ...
    /// finite RTT-derived bound (`neqo-transport`'s `default_max_age`), not
    /// "no limit".  See [`ClientSession::webtransport_set_datagram_max_age`].
    ///
    /// # Errors
    ///
    /// Returns `InvalidStreamId` if the session no longer exists.
