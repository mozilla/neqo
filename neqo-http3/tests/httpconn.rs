...
fn connect_peers(hconn_c: &mut Http3Client, hconn_s: &mut Http3Server) -> Option<Datagram> {
    ...
    let out = hconn_c.process_alloc(None, now()); // Initial
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now()); // Initial + Handshake
    let out = hconn_c.process_alloc(out.as_dgram_ref(), now()); // ACK
    mem::drop(hconn_s.process_alloc(out.as_dgram_ref(), now())); // consume ACK
    let authentication_needed = |e| matches!(e, Http3ClientEvent::AuthenticationNeeded);
    assert!(hconn_c.events().any(authentication_needed));
    hconn_c.authenticated(AuthenticationStatus::Ok, now());
    let out = hconn_c.process_alloc(None, now()); // Handshake
    assert_eq!(hconn_c.state(), Http3State::Connected);
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now()); // Handshake
    let out = hconn_c.process_alloc(out.as_dgram_ref(), now());
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now());
...
fn connect_peers_with_network_propagation_delay(
    ...
    let out = hconn_c.process_alloc(out.as_dgram_ref(), now); // ACK
    now += net_delay;
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now); // consume ACK
    ...
    assert_eq!(hconn_c.state(), Http3State::Connected);
    now += net_delay;
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now); // HANDSHAKE_DONE
    now += net_delay;
    let out = hconn_c.process_alloc(out.as_dgram_ref(), now); // Consume HANDSHAKE_DONE, send control streams.
    now += net_delay;
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now); // consume and send control streams.
    now += net_delay;
    let out = hconn_c.process_alloc(out.as_dgram_ref(), now); // consume control streams.
    (out.dgram(), now)
}
...
fn exchange_packets(client: &mut Http3Client, server: &mut Http3Server, out_ex: Option<Datagram>) {
    let mut out = out_ex;
    loop {
        out = client.process_alloc(out.as_ref(), now()).dgram();
        out = server.process_alloc(out.as_ref(), now()).dgram();
        if out.is_none() {
            break;
        }
    }
...
fn fetch() {
    ...
    let out = hconn_c.process_alloc(dgram.as_ref(), now());
    qtrace!("-----server");
    let out = hconn_s.process_alloc(out.as_dgram_ref(), now());
    mem::drop(hconn_c.process_alloc(out.as_dgram_ref(), now()));
    process_server_events(&hconn_s);
    let out = hconn_s.process_alloc(None, now());
...
fn fetch_noresponse_will_idletimeout() {
    ...
    assert_eq!(req, 0);
    hconn_c.stream_close_send(req).unwrap();
    let _out = hconn_c.process_alloc(dgram.as_ref(), now);
    qtrace!("-----server");
