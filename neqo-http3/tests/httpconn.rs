...
fn fetch_noresponse_healthy_path_stays_open() {
    ...
    // Drive both peers for several idle-timeout periods, delivering every
    // keepalive and its acknowledgement. No packet is ever lost, so the
    // consecutive-PTO count stays at zero and max_pto never trips.
    let mut now = now();
    let end = now + ConnectionParameters::DEFAULT_IDLE_TIMEOUT * 4;
    while now < end {
        // Exchange all keepalives and acknowledgements pending at this instant.
        loop {
            if let Some(d) = hconn_c.process_output(now).dgram() {
                if let Some(ack) = hconn_s.process(Some(d), now).dgram() {
                    hconn_c.process_input(ack, now);
                }
                continue;
            }
            if let Some(d) = hconn_s.process_output(now).dgram() {
                if let Some(ack) = hconn_c.process(Some(d), now).dgram() {
                    // `Http3Server` has no `process_input`, so feed the ack via `process`.
                    _ = hconn_s.process(Some(ack), now);
                }
                continue;
            }
            break;
        ...
        // Advance to whichever keepalive either peer schedules next.
        now += hconn_c
            .process_output(now)
            .callback()
            .min(hconn_s.process_output(now).callback());
    }
}
