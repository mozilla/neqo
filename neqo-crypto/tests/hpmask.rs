...
fn hpmask_hack() {
    init_db("./db");
    let key = SymKey::import(
        SymKeyTarget::Hkdf(TLS_AES_128_GCM_SHA256),
        &[
            0x94, 0xb9, 0x45, 0x2d, 0x2b, 0x3c, 0x7c, 0x7f, 0x6d, 0xa7, 0xfd, 0xd8, 0x59, 0x35,
            0x37, 0xfd, // Include extra bits, which should be trimmed off.
    ...
    .expect("can make key");
    let key = key
        .into_hpmask(TLS_AES_128_GCM_SHA256)
        .expect("can convert key");
    let input = &[
        0xc4, 0xc2, 0xa2, 0x30, 0x3d, 0x29, 0x7e, 0x3c, 0x51, 0x9b, 0xf6, 0xb2, 0x23, 0x86, 0xe3,
        0xd0,
    ];
