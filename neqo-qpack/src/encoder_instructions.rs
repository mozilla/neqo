...
use crate::qpack_helper::{IntReader, LiteralReader, ReadByte, Reader};
use crate::qpack_send_buf::QPData;
use crate::{Error, Prefix, Res, NO_PREFIX};
use neqo_common::{qdebug, qtrace};
use std::mem;

const ENCODER_CAPACITY_PREFIX: u8 = 0x20;
const ENCODER_INSERT_WITH_NAME_REF_STATIC_PREFIX: u8 = 0xC0;
const ENCODER_INSERT_WITH_NAME_REF_DYNAMIC_PREFIX: u8 = 0x80;
const ENCODER_INSERT_WITH_NAME_LITERAL_PREFIX: u8 = 0x40;
const ENCODER_DUPLICATE_PREFIX: u8 = 0x00;

// Encoder instructions prefix
const ENCODER_CAPACITY: Prefix = Prefix {
    prefix: ENCODER_CAPACITY_PREFIX,
    len: 3,
};
const ENCODER_INSERT_WITH_NAME_REF_STATIC: Prefix = Prefix {
    prefix: ENCODER_INSERT_WITH_NAME_REF_STATIC_PREFIX,
    len: 2,
};
const ENCODER_INSERT_WITH_NAME_REF_DYNAMIC: Prefix = Prefix {
    prefix: ENCODER_INSERT_WITH_NAME_REF_DYNAMIC_PREFIX,
    len: 2,
};
const ENCODER_INSERT_WITH_NAME_LITERAL: Prefix = Prefix {
    prefix: ENCODER_INSERT_WITH_NAME_LITERAL_PREFIX,
    len: 2,
};
const ENCODER_DUPLICATE: Prefix = Prefix {
    prefix: ENCODER_DUPLICATE_PREFIX,
    len: 3,
};
...
impl EncoderInstructionReader {
    ...
    fn decode_instruction_from_byte(&mut self, b: u8) {
        self.instruction = match b & MASK_2 {
            ENCODER_INSERT_WITH_NAME_REF_STATIC_PREFIX => {
                EncoderInstruction::InsertWithNameRefStatic {
                    index: 0,
                    value: Vec::new(),
                }
            }
            ENCODER_INSERT_WITH_NAME_REF_DYNAMIC_PREFIX => {
                EncoderInstruction::InsertWithNameRefDynamic {
                    index: 0,
                    value: Vec::new(),
                }
            }
            ENCODER_INSERT_WITH_NAME_LITERAL_PREFIX => EncoderInstruction::InsertWithNameLiteral {
                name: Vec::new(),
                value: Vec::new(),
            },
            0x0 => match b & MASK_3 {
                ENCODER_CAPACITY_PREFIX => EncoderInstruction::Capacity { value: 0 },
                ENCODER_DUPLICATE_PREFIX => EncoderInstruction::Duplicate { index: 0 },
                _ => unreachable!("The above patterns match everything."),
            },
            _ => unreachable!("The above patterns match everything."),
        };
