...
pub fn decode(input: &[u8]) -> Res<Vec<u8>> {
    ...
    let mut output = Vec::with_capacity(input.len()); // Huffman typically expands, so start with input size
...
pub fn encode(input: &[u8]) -> Vec<u8> {
    let mut output: Vec<u8> = Vec::with_capacity((input.len() * 5) / 8 + 1); // Huffman compresses ~37.5%, with buffer
    let mut left: u8 = 8;
    let mut saved: u8 = 0;
    for c in input {
