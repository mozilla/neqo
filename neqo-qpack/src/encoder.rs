...
mod tests {
    ...
    impl TestEncoder {
        ...
        pub fn send_instructions(&mut self, encoder_instruction: &[u8]) {
            ...
            let out = self.conn.process_alloc(None, now());
            let out2 = self.peer_conn.process_alloc(out.as_dgram_ref(), now());
            mem::drop(self.conn.process_alloc(out2.as_dgram_ref(), now()));
    ...
    fn recv_instruction(encoder: &mut TestEncoder, decoder_instruction: &[u8]) {
        encoder
            ...
            .unwrap();
        let out = encoder.peer_conn.process_alloc(None, now());
        mem::drop(encoder.conn.process_alloc(out.as_dgram_ref(), now()));
        assert!(encoder
            .encoder
    ...
    fn do_not_evict_entry_that_are_referred_only_by_the_same_header_blocked_encoding() {
        ...
        encoder
            ...
            .unwrap();
        let out = encoder.conn.process_alloc(None, now());
        mem::drop(encoder.peer_conn.process_alloc(out.as_dgram_ref(), now()));
        // receive an insert count increment.
        recv_instruction(&mut encoder, &[0x01]);
