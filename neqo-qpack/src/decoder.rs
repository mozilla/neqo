...
mod tests {
    ...
    fn recv_instruction(decoder: &mut TestDecoder, encoder_instruction: &[u8], res: &Res<()>) {
        _ = decoder
            ...
            .unwrap();
        let out = decoder.peer_conn.process_alloc(None, now());
        mem::drop(decoder.conn.process_alloc(out.as_dgram_ref(), now()));
        assert_eq!(
            decoder
                .decoder
    ...
    fn send_instructions_and_check(decoder: &mut TestDecoder, decoder_instruction: &[u8]) {
        ...
        let out = decoder.conn.process_alloc(None, now());
        mem::drop(decoder.peer_conn.process_alloc(out.as_dgram_ref(), now()));
