    ...
    T: Debug + Sized,
{
    ...
    fn auto_tune_inner(
        ...
        // This is because, when not application limited, in a steady state
        // transfer it takes 1 RTT after sending 1 / F bytes for the sender to
        // receive the next update. The sender is effectively limited to
        // [(F-1) / F] bytes per RTT.
...
mod test {
    ...
    fn auto_tuning_approximates_bandwidth_delay_product() -> Res<()> {
        ...
        for _ in 0..100 {
            ...
            let mut accrued_latency = Duration::ZERO;
            let mut last_accrued = now;
            loop {
                accrued_latency = accrued_latency.saturating_sub(now.duration_since(last_accrued));
                last_accrued = now;
                ...
                let sender_progressed = if sender_window > 0 {
                    ...
                    // Every time we send data we accrue some latency and push back later sends.
                    accrued_latency += time_to_send;
                    send_to_recv.push_back((now + accrued_latency + half_rtt, to_send));
                    true
                } else {
                    false
                };
