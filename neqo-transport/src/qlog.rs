...
use crate::packet::{DecryptedPacket, PacketType};
use crate::path::Path;
...
pub fn packet_received(qlog: &Option<NeqoQlogRef>, now: Instant, payload: &DecryptedPacket) {
    if let Some(qlog) = qlog {
                    ...
                    let qlogframe = match f {
                        ...
                        Frame::ConnectionClose {
                            ...
                            error_space: match error_code {
                                ...
                                frame::CloseError::Application(_) => {
        ...
        let packet_type = match payload.packet_type() {
