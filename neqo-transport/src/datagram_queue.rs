...
pub enum DatagramQueueOutcome {
    ...
    /// entry is an evicted datagram's tracking ID, or `None` if it was
    /// enqueued untracked; the caller reports a [`DatagramOutcome::Dropped`] event
...
/// must-not-starve rule for null-`[[SendOrder]]` sends. Not mirrored here yet: doing so touches
/// [`GroupQueue`] and every method that takes a `send_order`, and matters once `#3983` gives this
/// queue and `SendStreams` a shared `SendGroupId` to coordinate.
#[derive(Debug)]
pub struct DatagramQueue {
    ...
    /// Set once [`Self::enqueue`] returns anything but
    /// [`DatagramQueueOutcome::Ok`]; cleared, reporting a resume signal due,
    /// once a removal drops the queue back below *both* the high water mark
    /// and the byte budget. See [`Self::resume_if_unblocked`].
    blocked: bool,
    /// The charge [`Self::enqueue`] needed when it last set `blocked`. `1`
    /// for [`DatagramQueueOutcome::AboveWatermark`]/
    /// [`DatagramQueueOutcome::Overflowed`], which admit the datagram, so any
    /// freed byte is progress; the refused datagram's actual charge for
    ...
    /// freed up. Meaningless while `blocked` is `false`.
    blocked_charge: usize,
...
impl DatagramQueue {
    ...
    pub fn enqueue(
        ...
        while self.total_bytes + new_charge > self.max_queued_bytes {
            ...
            if self
                .lowest_priority_key()
                .is_some_and(|victim| send_order < victim.0)
            {
                outranked = true;
                break;
            }
        ...
        if outranked && dropped.is_empty() {
            ...
            // room for it.
            self.blocked = true;
            self.blocked_charge = new_charge;
    ...
    /// drains back below the high water mark and has freed at least
    /// `blocked_charge`, for the caller to fire a resume signal. `false`
    /// every other time, including every call while the queue was never
    /// blocked.
    ///
    ...
    pub fn resume_if_unblocked(&mut self) -> bool {
        if self.blocked
            ...
            && (self.is_empty()
                || self.max_queued_bytes.saturating_sub(self.total_bytes) >= self.blocked_charge)
        {
            self.blocked = false;
            true
        } else {
...
mod tests {
    ...
    fn byte_budget_admits_equal_priority_newcomer_across_groups() {
        ...
        q.enqueue(vec![0, 1], Some(1), t, g(2), 1);
        q.enqueue(vec![0, 2], Some(2), t, g(1), 1);
        assert_eq!(q.len(), 2);
    ...
    fn rejection_of_a_large_write_does_not_resume_until_it_would_fit() {
        ...
        q.enqueue(vec![1], Some(1), t, g(0), 100);
        q.enqueue(vec![2], Some(2), t, g(0), 100);
        assert_eq!(
