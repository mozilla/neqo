...
pub enum DatagramQueueOutcome {
    ...
    /// number evicted, tracked or not, for aggregate stats; this variant
    /// deliberately does not name which tracked IDs were among them (see
    /// `DatagramQueue::enqueue`'s doc for why).
...
impl DatagramQueue {
    ...
    /// budget. It is *not* the mapping for `outgoingHighWaterMark == 0`,
    ...
    /// [`DatagramQueueOutcome::Rejected`] the caller reports a
    /// [`DatagramOutcome::Dropped`] event for `id` itself, if tracked.
    /// [`DatagramQueueOutcome::Overflowed`] deliberately does not identify
    /// which already-queued datagrams were evicted - nothing outside this
    /// crate resolves a tracking ID for one of *those* back to anything
    /// actionable, so the aggregate count in `dropped` is all that outcome
    /// carries.
    ///
    ...
    pub fn enqueue(
        ...
        if outranked && evicted_count == 0 {
            ...
            // outcome but `Ok`: arm one, or it waits forever. Combined with
            // any outstanding charge rather than overwritten - `new_charge`
            // is the exact charge *this* write needs, so a smaller refused
            // write elsewhere must not relax a larger one still waiting to
            // be retried.
            self.blocked = Some(self.blocked.map_or(new_charge, |c| c.max(new_charge)));
        ...
        let outcome = if evicted_count > 0 {
            self.blocked = Some(self.blocked.unwrap_or(1));
            ...
            // The datagram was admitted, so any freed byte is progress for
            // the next write, not a specific charge to wait for - but never
            // relax a charge an earlier `Rejected` is still waiting on.
            self.blocked = Some(self.blocked.unwrap_or(1));
...
mod tests {
    ...
    fn enq(q: &mut DatagramQueue, data: Vec<u8>, id: u64, t: Instant, gid: u64, order: SendOrder) {
        drop(q.enqueue(data, Some(id), t, g(gid), order));
    }
    ...
    fn expiry_prunes_an_emptied_group_without_stalling_the_others() {
        ...
        assert_eq!(q.expire(t1, NO_DEFAULT), vec![Some(1)]);
        assert_eq!(q.len(), 1);
