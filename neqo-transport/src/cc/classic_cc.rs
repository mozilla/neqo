...
impl<S, T> Display for ClassicCongestionControl<S, T>
where
    S: SlowStart,
    T: WindowAdjustment,
{
    ...
    T: WindowAdjustment,
{
    ...
    fn on_packets_acked(
        ...
        if self.congestion_window < self.ssthresh {
            ...
            self.congestion_window += result.cwnd_increase;
            self.acked_bytes = result.unused_acked_bytes;
...
mod tests {
    ...
    use super::{ClassicCongestionControl, SlowStart, WindowAdjustment, PERSISTENT_CONG_THRESH};
    use crate::{
        cc::{
            classic_cc::State,
            cubic::Cubic,
            new_reno::NewReno,
            tests::{make_cc_cubic, make_cc_newreno, RTT},
            ClassicSlowStart, CongestionControl, CongestionEvent, CWND_INITIAL_PKTS,
        },
        packet,
        recovery::{self, sent},
        rtt::RttEstimate,
    ...
    ///
    /// Send packets (1, 2) --> `SlowStart`, no events
    /// Lose packets (1, 2) --> `RecoveryStart`, 1 event
    /// Send packet (3)     --> `Recovery`, 1 event
    /// Ack packet (3)      --> `CongestionAvoidance`, 1 event
    /// Ack packet (1)      --> `CongestionAvoidance`, 1 event, not a spurious event as not all lost
    ///                         packets were recovered
    /// Ack packet (2)      --> all lost packets have been recovered so now we've detected a
