...
fn tcp_phase() {
    ...
    let mut now = now();
    let start_time = now;
    // helper variables to remember the next packet number ro be send/acked.
    ...
    for _ in 0..num_tcp_increases {
        ...
        let acks = (f64::try_from(i32::try_from(cwnd_rtt_start).unwrap()).unwrap()
    ...
    // Make sure that the increase is not according to TCP equation, i.e tha it took
    // less than RTT / CUBIC_ALPHA.
    let expected_ack_tcp_increase = (f64::try_from(u32::try_from(cwnd_rtt_start).unwrap()).unwrap()
        / MAX_DATAGRAM_SIZE_F64
        / CUBIC_ALPHA)
        .round() as u64;
    ...
    let elapse_time = now - start_time - time_increase; // "- time_increase" because it is added above.
    ...
    now -= time_increase; // "- time_increase" because it is added above.
    let expected_ack_tcp_increase2 =
        (f64::try_from(u32::try_from(cwnd_rtt_start_after_tcp).unwrap()).unwrap()
            / MAX_DATAGRAM_SIZE_F64
            / CUBIC_ALPHA)
            .round() as u64;
    ...
    // W_cubic(elapse_time + t_to_increase) - W_cubis(elapse_time) = MAX_DATAGRAM_SIZE =>
    // CUBIC_C * (elapse_time + t_to_increase)^3 * MAX_DATAGRAM_SIZE + CWND_INITIAL -
    //     CUBIC_C * elapse_time^3 * MAX_DATAGRAM_SIZE + CWND_INITIAL = MAX_DATAGRAM_SIZE =>
    // t_to_increase = cbrt((1 + CUBIC_C * elapse_time^3) / CUBIC_C) - elapse_time
    // (t_to_increase is in seconds)
    // number of ack needed is t_to_increase / time_increase.
    let expected_ack_cubic_increase =
        ((((1.0 + CUBIC_C * (elapse_time).as_secs_f64().powi(3)) / CUBIC_C).cbrt()
            - elapse_time.as_secs_f64())
...
fn cubic_phase() {
    ...
    for _ in 0..num_rtts_w_max {
        ...
        assert_close(cubic.cwnd(), expected);
    }
    assert_eq!(cubic.cwnd(), CWND_INITIAL_10);
}

fn assert_close(cwnd: usize, expects_cwnd: usize) {
    if cwnd >= expects_cwnd {
        assert!(cwnd <= expects_cwnd + MAX_DATAGRAM_SIZE);
    } else {
        assert!(expects_cwnd <= cwnd + MAX_DATAGRAM_SIZE);
    }
}
...
fn congestion_event_slow_start() {
    ...
    assert!((cubic.last_max_cwnd() - 0.0).abs() < f64::EPSILON);
    ...
    assert!(
        (cubic.last_max_cwnd() - (CWND_INITIAL_F64 + MAX_DATAGRAM_SIZE_F64)).abs() < f64::EPSILON
    );
...
fn congestion_event_congestion_avoidance() {
    ...
    // Set last_max_cwnd to someting smaller than cwnd so that the fast convergence is not triggered.
    ...
    assert!((cubic.last_max_cwnd() - CWND_INITIAL_F64).abs() < f64::EPSILON);
...
fn congestion_event_congestion_avoidance_2() {
    ...
    // Set last_max_cwnd to someting higher than cwnd so that the fast convergence is triggered.
    cubic.set_last_max_cwnd(CWND_INITIAL_10_F64);

    let _ = fill_cwnd(&mut cubic, 0, now());
    ack_packet(&mut cubic, 0, now());

    assert!((cubic.last_max_cwnd() - CWND_INITIAL_10_F64).abs() < f64::EPSILON);
    assert_eq!(cubic.cwnd(), CWND_INITIAL);
    ...
    assert!(
        (cubic.last_max_cwnd() - CWND_INITIAL_F64 * CUBIC_FAST_CONVERGENCE).abs() < f64::EPSILON
    );
    assert_eq!(cubic.cwnd(), CWND_AFTER_LOSS);
}
