...
/// This only works at the client end as our server `Connection` expects to be
/// configured properly, with Version Negotiation packets generated by `Server`.
fn one_version_only(version: QuicVersion) {
    ...
    let mut server =
        new_server(ConnectionParameters::default().versions(version, QuicVersion::all()));
...
fn version_negotiation_downgrade() {
    const DOWNGRADE: QuicVersion = QuicVersion::Draft29;
    ...
    let mut server =
        new_server(ConnectionParameters::default().versions(DOWNGRADE, QuicVersion::all()));
...
fn invalid_server_version() {
    let mut client = new_client(
        ConnectionParameters::default().versions(QuicVersion::Version1, QuicVersion::all()),
    );
    let mut server = new_server(
        ConnectionParameters::default().versions(QuicVersion::Version2, QuicVersion::all()),
    );
