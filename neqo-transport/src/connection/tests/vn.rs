...
fn one_version_only(version: Version) {
    let mut client = new_client(ConnectionParameters::default().versions(version, vec![version]));
    let mut server = new_server(ConnectionParameters::default().versions(version, Version::all()));
...
#[test]
fn just_v1_client() {
    one_version_only(Version::Version1);
}

#[test]
fn just_v2_client() {
    one_version_only(Version::Version2);
}
