...
use super::{
    assert_error, connect_force_idle, connect_with_rtt, default_client, default_server,
    get_last_token, handshake, maybe_authenticate, send_something, split_datagram, AT_LEAST_PTO,
    DEFAULT_STREAM_DATA,
};
...
fn reorder_05rtt_with_0rtt() {
    ...
    let token = get_last_token(&mut client).unwrap().token;
    let mut client = default_client();
