...
fn pto_initial() {
    ...
    let mut client = default_client();
    let pkt1 = client.process_alloc(None, now).dgram();
    assert!(pkt1.is_some());
    ...
    now += delay;
    let pkt2 = client.process_alloc(None, now).dgram();
    assert!(pkt2.is_some());
    ...
    let mut server = default_server();
    let out = server.process_alloc(pkt1.as_ref(), now).dgram();
    assert!(out.is_some());
...
fn pto_handshake_complete() {
    ...
    let pkt = client.process_alloc(None, now).dgram();
    assert_initial(pkt.as_ref().unwrap(), false);
    let cb = client.process_alloc(None, now).callback();
    assert_eq!(cb, Duration::from_millis(300));

    now += HALF_RTT;
    let pkt = server.process_alloc(pkt.as_ref(), now).dgram();
    assert_initial(pkt.as_ref().unwrap(), false);

    now += HALF_RTT;
    let pkt = client.process_alloc(pkt.as_ref(), now).dgram();
    ...
    now += HALF_RTT * 6;
    let pkt3 = client.process_alloc(None, now).dgram();
    ...
    let ack = server.process_alloc(pkt1.as_ref(), now).dgram();
    assert!(ack.is_some());
    ...
    now += cb;
    let out = client.process_alloc(None, now).dgram();
    assert!(out.is_some());
}
...
fn pto_handshake_frames() {
    ...
    qdebug!("---- client: generate CH");
    let mut client = default_client();
    let pkt = client.process_alloc(None, now);

    now += Duration::from_millis(10);
    qdebug!("---- server: CH -> SH, EE, CERT, CV, FIN");
    let mut server = default_server();
    let pkt = server.process_alloc(pkt.as_dgram_ref(), now);

    now += Duration::from_millis(10);
    qdebug!("---- client: cert verification");
    let pkt = client.process_alloc(pkt.as_dgram_ref(), now);

    now += Duration::from_millis(10);
    mem::drop(server.process_alloc(pkt.as_dgram_ref(), now));
...
fn handshake_ack_pto() {
    ...
    assert!(s1.is_some());
    let s2 = server.process_alloc(None, now).dgram();
    assert!(s1.is_some());
    ...
    let c2 = client.process_alloc(s2.as_ref(), now).dgram();
...
fn loss_recovery_crash() {
    ...
    let dgram = server.process_alloc(None, now + AT_LEAST_PTO).dgram();
...
fn ack_after_pto() {
    ...
    let ack = client.process_alloc(Some(&dgram), now).dgram();
    assert!(ack.is_some());
...
fn lost_but_kept_and_lr_timer() {
    ...
    now += RTT / 2;
    let ack = server.process_alloc(Some(&p2), now).dgram();
    assert!(ack.is_some());
    ...
    now += RTT / 2;
    let res = client.process_alloc(ack.as_ref(), now);
    ...
    let ack = server.process_alloc(Some(&p4), now).dgram();
    assert!(ack.is_some());
...
fn loss_time_past_largest_acked() {
    ...
    // Start the handshake.
    let c_in = client.process_alloc(None, now).dgram();
    now += RTT / 2;
    let s_hs1 = server.process_alloc(c_in.as_ref(), now).dgram();
    ...
    assert!(s_pto < RTT);
    let s_hs2 = server.process_alloc(None, now + s_pto).dgram();
    assert!(s_hs2.is_some());
    let s_pto = server.process_alloc(None, now).callback();
    assert_ne!(s_pto, Duration::from_secs(0));
    assert!(s_pto < RTT);
    let s_hs3 = server.process_alloc(None, now + s_pto).dgram();
    ...
    now += RTT / 2;
    let c_hs1 = client.process_alloc(s_hs1.as_ref(), now).dgram();
    assert!(c_hs1.is_some()); // This comes first, so it's useless.
    maybe_authenticate(&mut client);
    let c_hs2 = client.process_alloc(None, now).dgram();
    ...
    let s_ack = server.process_alloc(c_hs2.as_ref(), now).dgram();
    ...
    now += RTT / 2;
    let _c_ack = client.process_alloc(Some(&s_hs_ack), now).dgram();
    // This ACK triggers an immediate ACK, due to an ACK loss during handshake.
    let c_ack = client.process_alloc(None, now).dgram();
...
fn trickle(sender: &mut Connection, receiver: &mut Connection, mut count: usize, now: Instant) {
    ...
    while count > 0 {
        ...
        let dgram = sender.process_alloc(maybe_ack.as_ref(), now).dgram();

        maybe_ack = receiver.process_alloc(dgram.as_ref(), now).dgram();
...
fn ping_with_ack(fast: bool) {
    ...
    if let Output::Callback(t) = sender.process_alloc(None, now) {
        assert_eq!(t, DEFAULT_ACK_DELAY);
        assert!(sender.process_alloc(None, now + t).dgram().is_some());
    }
...
fn fast_pto_persistent_congestion() {
    ...
    now += DEFAULT_RTT / 2;
    let ack = server.process_alloc(Some(&dgram), now).dgram();
    now += DEFAULT_RTT / 2;
