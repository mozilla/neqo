...
fn closing_timers_interation() {
    ...
    client.close(now, 0, "");
    let client_close = client.process_alloc(None, now).dgram();
...
fn closing_and_draining() {
    ...
    let client_close2 = client.process_alloc(Some(&p3), now()).dgram();
    assert_eq!(
    ...
    let end = client.process_alloc(None, now() + client_close_timer);
    ...
    let server_close = server.process_alloc(client_close.as_ref(), now()).dgram();
    assert!(server.state().closed());
    ...
    let end = server.process_alloc(None, now() + server_close_timer);
    assert_eq!(end, Output::None);
    assert_eq!(
        *server.state(),
