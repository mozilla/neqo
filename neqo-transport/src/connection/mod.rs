...
impl Connection {
    ...
    pub fn process_output(&mut self, now: Instant) -> Output {
        self.process_multiple_output(now, 1)
            .try_into()
            .expect("TODO: set max_datagram to 1 thus expect a single datagram only")
    }

    #[must_use = "OutputBatch of the process_output function must be handled"]
    ...
    pub fn process<A: AsRef<[u8]> + AsMut<[u8]>>(
        ...
        self.process_multiple(dgram, now, 1)
            .try_into()
            .expect("TODO: set max_datagram to 1 thus expect a single datagram only")
    }
    ...
    fn output_dgram_batch_on_path(
        ...
        while num_segments < max_datagrams {
            // TODO: Safeguard for now to not send anything larger than the first packet or 64k GSO
            // limit.
            if num_segments > 0
                && min(
