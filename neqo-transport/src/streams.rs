...
impl Streams {
    ...
    /// Raise the local receive limit for incoming streams of `stream_type`,
    /// advertised to the peer via `MAX_STREAMS`. Monotonic: has no effect if
    /// `max` is not greater than the current limit.
