...
use std::{io::Error, net::IpAddr, ptr};
...
pub fn get_interface_mtu(remote_ip: &IpAddr) -> Result<u32, Error> {
    ...
    use libc::{freeifaddrs, getifaddrs, if_data, ifaddrs, sockaddr_in, sockaddr_in6};

    // Make a new socket that is connected to the remote address. We use this to learn which local
    // address is chosen by routing.
    ...
    // First, find the name of the interface with the the local IP address determined above. Then,
    // in a second loop below, find the MTU of that interface. We need to do two loops, because
    // `getifaddrs` returns one entry per interface and link type, and the IP addresses are in the
    // AF_INET/AF_INET6 entries for an interface, whereas the MTU is (only) in the AF_LINK entry,
    // whose `ifa_addr` contains MAC address information, not IP address information.
    let mut cursor = ifap;
    let iface = loop {
        if cursor.is_null() {
            break None;
