...
#[test]
fn gso() {
    let (mut client, server) = test_fixture::connect();
    ...
    let out = client.process2(None::<Datagram<Vec<u8>>>, now()).dgram().unwrap();

    assert_eq!(out.segment_size,  1232);
    assert_eq!(out.d.len(), 2108);
}
