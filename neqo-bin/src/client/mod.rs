...
use neqo_transport::{AppError, CloseReason, ConnectionId, OutputTrain, Version};
...
trait Client {
    fn process_output_train(&mut self, now: Instant, max_datagrams: usize) -> OutputTrain;
...
impl<'a, H: Handler> Runner<'a, H> {
    ...
    async fn process_output(&mut self) -> Result<(), io::Error> {
        loop {
            match self
                .client
                .process_output_train(Instant::now(), self.socket.max_gso_segments())
            {
                OutputTrain::Datagram(dgram) => loop {
