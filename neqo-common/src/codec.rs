...
impl Buffer for Cursor<&mut [u8]> {
    ...
    fn truncate(&mut self, len: usize) {
        let new_len = usize::try_from(self.position()).expect("Buffer length does not exceed usize");
        if len < new_len {
